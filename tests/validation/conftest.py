...
# ---------------------------------------------------------------------------
# mfd-connect <= 7.16.0 does not support proxy_command in SSHConnection.
# Verified against latest release; re-check after upgrading mfd-connect.
_orig_ssh_init = None
_orig_ssh_connect = None


def _patch_ssh_connection_proxy_command():
    """Intercept proxy_command from topology connection_options.

    Injects a paramiko ProxyCommand socket so that _connect() tunnels
    through the SOCKS/netcat proxy specified in the topology YAML.
    """
