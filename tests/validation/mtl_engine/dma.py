...
def bind_dma_to_vfio(host, pci_address: str) -> bool:
    ...
    try:
        ...
        bound_drv = result.stdout.strip() if result and result.stdout else "none"
        if bound_drv != "vfio-pci":
            logger.error(
                f"Failed to bind DMA {pci_address} to vfio-pci "
                f"on {host.name} (current driver: {bound_drv})"
            )
            return False
        ...
        # Log DMA binding status via dpdk-devbind for easy diagnostics
        try:
            status_result = host.connection.execute_command(
            ...
            if devbind_status:
                logger.info(f"DMA {pci_address} on {host.name}: {devbind_status}")
        except Exception:
            pass  # diagnostic only, don't fail the bind
