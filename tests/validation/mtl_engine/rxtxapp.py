...
class RxTxApp(Application):
    ...
    def _create_rxtxapp_command_and_config(self) -> tuple:
        ...
        # Build command line
        cmd_parts = ["sudo", self.get_executable_path(), "--config_file", config_file_path]
        ...
        for universal_param, rxtx_param in RXTXAPP_CMDLINE_PARAM_MAP.items():
            ...
            if universal_param == "test_time" and not self.was_user_provided("test_time"):
                continue
    ...
    def _create_rxtxapp_config_dict(self) -> dict:
        ...
        nic_port_list = self.universal_params.get("nic_port_list") or ([nic_port] if nic_port else [])
        ...
        try:
            ...
            if test_mode == "unicast":
                ...
                config["tx_sessions"][0]["dip"][0] = ip_pools.rx[0] if ip_pools.rx else None
                config["rx_sessions"][0]["ip"][0] = ip_pools.tx[0] if ip_pools.tx else None
            elif test_mode == "multicast":
                config["interfaces"][0]["ip"] = ip_pools.tx[0] if ip_pools.tx else None
                config["tx_sessions"][0]["dip"][0] = ip_pools.rx_multicast[0] if ip_pools.rx_multicast else None
                config["rx_sessions"][0]["ip"][0] = ip_pools.rx_multicast[0] if ip_pools.rx_multicast else None
            ...
            if len(nic_port_list) > 1:
                ...
                if test_mode == "unicast":
                    config["interfaces"][1]["ip"] = ip_pools.rx[0] if ip_pools.rx else None
                elif test_mode == "multicast":
                    config["interfaces"][1]["ip"] = ip_pools.rx[0] if ip_pools.rx else None
    ...
    def prepare_execution(self, build: str, host=None, **kwargs):
        ...
        if connection_type == "LocalConnection":
            ...
            python_write_cmd = f"python3 -c \"import sys; open('{config_file_path}', 'w').write(sys.stdin.read())\" <<< '{escaped_json}'"
